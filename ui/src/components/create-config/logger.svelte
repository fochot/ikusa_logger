<script lang="ts">
	...
	$: {
		if (tracked_logs.length > 0) {
			logs_changed(tracked_logs);
		} else {
			scroll(true);
		}
	}

	function logs_changed(source_logs: LogType[]) {
		auto_scroll && setTimeout(scroll);

		if (source_logs.length < 50 || source_logs.length % 100 === 0) {
			const detected_kill_offsets = find_kill_offset(source_logs);
			if (detected_kill_offsets.length > 0) {
				possible_kill_offsets = detected_kill_offsets;
			}
			calculate_config(source_logs);
		} else {
			write_live_output();
		}
	}

	async function calculate_config(source_logs: LogType[]) {
	...
	function find_kill_offset(source_logs: LogType[]) {
	...
	async function write_live_output() {
		if (!live_output_path || tracked_logs.length === 0) return;
...
<div class="flex flex-col gap-2 items-center w-full h-full relative">
	<div class="flex gap-1 items-center justify-start w-full px-1">
		<p class="text-xs sm:text-sm text-gray-300">
			{tracked_logs.length} tracked{#if personal_only} / {logs.length} detected{/if}
		</p>
		<div class="ml-2">
	...
	<div class="w-full flex gap-2 pb-14" style="height: {height}px;">
		...
		>
			{#if loading && tracked_logs.length === 0 && (!personal_only || personal_family_name.trim())}
				<div class="absolute inset-0 flex justify-center items-center mb-14">
					<LoadingIndicator />
				</div>
			{:else if tracked_logs.length === 0}
				<div
					class="absolute inset-0 flex flex-col gap-1 items-center justify-center px-6 text-center"
				>
					{#if personal_only && !personal_family_name.trim()}
						<p class="text-gray-400">Enter your family name to start filtering.</p>
					{:else if personal_only && logs.length > 0}
						<p class="text-gray-400">No fights involving {personal_family_name.trim()} yet.</p>
						<p class="text-xs text-gray-500">{logs.length} unrelated candidates ignored.</p>
					{:else}
						<p class="text-gray-400">Waiting for logs...</p>
						<p class="text-xs text-gray-500 line-clamp-2" title={status_message}>
							{status_message}
						</p>
					{/if}
				</div>
		...
		<div class="flex-1 flex flex-col gap-2 text-xs h-full overflow-y-auto">
			...
			<div class="rounded-lg border border-gray-700 p-2.5">
				...
				<p class="text-gray-500 mb-1.5">
					{personal_only ? 'Only matching fights are tracked.' : 'All detected fights are tracked.'}
				</p>
